...
3. ~~**`preview/ds3-preview.html`** — memory note บอก "do not treat as canonical" แต่ session ก่อนหน้านี้ user ก็เปิดดู → ควร mark สถานะให้ชัด (banner ใน file header)~~ **RESOLVED 2026-05-18:** เพิ่ม 2 banners แล้ว — (1) HTML comment block ใน `<head>` อธิบาย scope + DO / DON'T สำหรับ AI agents และ devs ที่อ่าน source, (2) visible sticky `<aside>` banner ที่ top ของ body สำหรับคนเปิดดู rendered page. ทั้งสองชี้ไปที่ `@uxuissk/design-system-core` npm + Suki Designer skill เป็น canonical source.
