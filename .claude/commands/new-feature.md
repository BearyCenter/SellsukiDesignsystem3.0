...
Create these files following the exact pattern of `src/repository/order_repository/`:
...
**`dummy.go`** — no-op stub implementing all interface methods, returning zero values. Used for development without a real DB.
...
In the helper/init file that calls `use_case.New(...)`:
- Instantiate `<domain>_repository.NewDummy()` (or `NewGormPostgres(...)` if DB is relevant)
- Pass it to `use_case.New(...)`
...
2. Add handler methods to `route.go` following the `routeDemoV1` pattern:
   - Extract identity with `helper.GetIdentityFromHeader(c)`
   - Parse body with `c.BodyParser(&req)`
   - Call use case with `c.UserContext()`
   - Map errors with `helper.ErrorHandler(c, err)`
