...
import "../../../src/main";
import { DropdownV2 } from "../../../src/main";
import { AutoLitProperty, baseArgsTypes } from "../helper";

type DropdownWithLabel = AutoLitProperty<DropdownV2> & {
  placeholder: string;
};
...
const meta = {
  title: "Example/Dropdown-v2",
  tags: ["autodocs"],
  render: ({ ...args }) => {
    const [{}, updateArgs] = useArgs();

    return html`<ssk-dropdown-v2
      ${spread({ ...args })}
      @change=${(e: any) => {
        action("@change")(e);
    ...
    >
      <ssk-dropdown-button-v2 slot="selected">
        <ssk-dropdown-preview-v2 value=${ifDefined(args["value"])}>
          <ssk-icon slot="prefix" name=${ifDefined(args["value"])}></ssk-icon>
          ${args["value"] || args["placeholder"]}
        </ssk-dropdown-preview-v2>
      </ssk-dropdown-button-v2>

      <ssk-dropdown-options-container-v2>
        ${options.map((option) => {
          return html`<ssk-dropdown-option-v2 value=${option}>
            <ssk-icon name=${option} slot="prefix"></ssk-icon>
            ${option}
          </ssk-dropdown-option-v2>`;
        })}
      </ssk-dropdown-options-container-v2>
    </ssk-dropdown-v2>`;
  },
