...
export const ALLOWED_BRANDS = ["patona", "ccs3", "oc2plus"] as const;
...
function rule1_rootIsAppShellProvider(body: string): Violation | null {
  ...
  if (!trimmed.startsWith("<ssk-app-shell-provider")) {
    return {
      ...
      message:
        "Root element must be <ssk-app-shell-provider>. Got: " +
        trimmed.slice(0, 80) +
        "…",
    };
  }
...
function rule2_brandWhitelistAndMatch(
  body: string,
  meta: ContractMeta,
): Violation[] {
  ...
  const brand = m[1];
  if (!(ALLOWED_BRANDS as readonly string[]).includes(brand)) {
    violations.push({
      rule: 2,
...
function rule3_noHexLiteralsOutsideVar(bodyNoVar: string): Violation[] {
...
function rule4_noFontSizeBelow18(body: string): Violation[] {
...
function rule5_customTagsMustBeSskPrefix(body: string): Violation[] {
  const tags = [...body.matchAll(/<([a-z][a-z0-9]*-[a-z0-9-]*)\b/g)];
  const offenders = new Set<string>();
  for (const m of tags) {
    if (!m[1].startsWith("ssk-")) offenders.add(m[1]);
  }
...
function rule6_noTailwindForbiddenSize(body: string): Violation[] {
...
function rule7_buttonUsesVariantTone(body: string): Violation[] {
  const violations: Violation[] = [];
  ...
  for (const btn of buttons) {
    if (LEGACY.test(btn)) {
      violations.push({
        ...
        message:
          "<ssk-button> must use variant+tone, not legacy themeColor/backgroundColor/fontSize",
        snippet: btn,
      });
    }
 ...
 */
export function compareBrandSwitching(
  htmlA: string,
  htmlB: string,
): { ok: boolean; reason?: string } {
  const normalize = (h: string) =>
    stripComments(h).replace(
  ...
  return {
    ok: false,
    reason:
      "Templates differ beyond just the brand attribute. Brand-switching " +
